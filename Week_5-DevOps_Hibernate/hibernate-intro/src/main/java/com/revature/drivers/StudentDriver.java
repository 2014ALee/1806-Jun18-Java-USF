...
//			session.getTransaction().commit();
			...
			// Query for students with last name 'Singleton' OR first name 'Blake'
			System.out.println("Retrieving all students with last name 'Singleton' OR first name 'Blake'");
			Query query = session.createQuery("from Student s where s.lastName = :lastName OR s.firstName = :firstName");
			query.setParameter("lastName", "Singleton");
			query.setParameter("firstName", "Blake");
