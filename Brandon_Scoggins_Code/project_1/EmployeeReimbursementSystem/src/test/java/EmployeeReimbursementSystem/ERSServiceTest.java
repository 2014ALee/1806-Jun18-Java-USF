...
public class ERSServiceTest {
	...
	public void testLogInUserGoodUserNameAndGoodPassword() {
		...
		ERSUser admin = new ERSUser(61, "admin", "admin", "admin", "admin", "admin@admin.com", 2);
	...
	public void testRegisterExistingUserName() {
		ERSService service = new ERSService();
		ERSUser admin = new ERSUser(61, "admin", "admin", "admin", "admin", "newadmin@admin.com", 2);
		assertEquals("Registering an existing user should fail", false, Boolean.parseBoolean(service.registerUser(admin)[0]));
	}
	
	@Test
	public void testRegisterExistingEmail() {
		ERSService service = new ERSService();
		ERSUser admin = new ERSUser(61, "newadmin", "admin", "admin", "admin", "admin@admin.com", 2);
		assertEquals("Registering an existing user should fail", false, Boolean.parseBoolean(service.registerUser(admin)[0]));
	}
}
