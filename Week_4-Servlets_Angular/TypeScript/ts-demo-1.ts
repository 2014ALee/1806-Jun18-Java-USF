...
/* 
    ...
    the console).
*/
let counter = 5; // hovering over the word 'count' will show the assigned data type: 'number' 
counter = 'a';
...
*/
let x; // hovering over 'a' we can see that uninitialized variables are of type 'any'
...
let message;
message = 'abc';
let endsWithC = (<string>message).endsWith('c');    // type assertion (method 1)
let endsWithD = (message as string).endsWith('d');  // type assertion (method 2)
...
let u: number[] = [1, 2, 3, 4, 5];
let v: string[] = ['Hi', 'my', 'name', 'is', 'Slim', 'Shady'];
let w: any[] = [1, true, 'a', {name: 'John'}, [1, 2, 5], function() {
